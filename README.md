...
`🔧 手动选择` —— 它是 `select` 加 `filter`，列出订阅里全部真实节点
（已排除「剩余流量」「到期时间」这类信息类伪节点）。
...
它读订阅文件里每个节点的 `network` 字段作为真实依据，逐个检查三份配置里的直连组，
混进中转或组为空就返回 1。订阅文件含服务器地址与凭据，只在本地读取，不写出任何
字段，也不进仓库——Clash Verge 的 `profiles` 目录里就有现成的。
